...
void updateMedoids(vector<point>& points, vector<point>& medoids, int* clusters, const int nthread) {
    ...
    for (size_t i = 0; i < medoids.size(); i++) {
        ...
        for (size_t j = 0; j < points.size(); j++) {
            if (clusters[j] == i) { // Check if the point belongs to the current cluster
                ...
                for (size_t k = 0; k < points.size(); k++) {
                    if (clusters[k] == i)
                        totalDistance += points[j].calculateDistance(points[k]); // Calculate total distance to all other points in the same cluster
                }
        ...
        if (min_index != -1)
            medoids[i] = points[min_index]; // Update medoid
    }
}
