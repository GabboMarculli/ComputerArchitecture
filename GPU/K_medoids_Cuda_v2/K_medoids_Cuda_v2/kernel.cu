...
__global__ void updateMedoids(Point* d_p, int np, Point* d_m, int nc, int* d_c, double* d_distances, int nt) {
    ...
    for (int i = blockIdx.x; i < np; i += gridDim.x) {
        ...
        sharedDistances[threadIdx.x] = distance;
        ...
        if (threadIdx.x == 0) {
            ...
            for (int k = 0; k < blockDim.x; k++) {
                totalDistance += sharedDistances[k];
                sharedDistances[k] = 0.0;
            }
            d_distances[i] = totalDistance;
        }
    }
}
...
long long kMedoids(vector<Point> p, int np, int nc, int nt, int nb) {
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        ...
        updateMedoids << <blocksPerGrid, threadsPerBlock, nt * sizeof(double) >> > (d_p, np, d_m, nc, d_c, d_distances, nt);
        cudaDeviceSynchronize();
...
int main(int argc, char* argv[])
{
    ...
    int arr_np[] = { 10000 };
    int n_t[] = { 32 };
    int n_b[] = { 4,8,16,32,64,128,256,512,1024,2048,4096,8192};
    // Parse command line arguments
    for (int i_np = 0; i_np < 1; i_np++) {
        for (int i_t = 0; i_t < 1; i_t++) {
            for (int i_b = 0; i_b < 12; i_b++) {
                int np = arr_np[i_np]; // std::atoi(argv[1]); // Number of points
                ...
                for (int i = 0; i < NUM_EXECUTIONS; i++) {
                    long long execution_time = kMedoids(p, np, nc, n_t[i_t], n_b[i_b]);
                    total += execution_time;
                    values[i] = execution_time;
                }
                // Calculate the arithmetic mean of execution times
                long long mean = total / NUM_EXECUTIONS;
                cout << np << "," << nc << "," << n_t[i_t] << "," << n_b[i_b] << "," << total << "," << mean << "," << standardDeviation(values, mean) << ',' << min_value(values) << ',' << max_value(values) << endl;
                ...
                outfile << np << "," << nc << "," << n_t[i_t] << "," << n_b[i_b] << "," << total << "," << mean << ","
