...
void calculateClusters(vector<point>& points, vector<point>& medoids, int* clusters, const int nthread) {
...
// - nthread: Number of threads for parallelization
void updateMedoids(vector<point>& points, vector<point>& medoids, int* clusters, const int nthread) {   
            ...
            for(int i = 0; i < medoids.size(); i++){
                if(clusters[j] == i){
                    ...
                    for(int k = 0; k < points.size(); k++){
                        if(clusters[k] == i)
                            distanceToCluster += points[i].calculateDistance(points[k]);
                    }
...
long long kMedoids(vector<point>& points, const int nClusters, const int nthread) {
    ...
    for (iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        updateMedoids(points, medoids, clusters, nthread);
    }
