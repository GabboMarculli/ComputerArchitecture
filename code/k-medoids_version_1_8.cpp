...
class point{
    ...
    public:
        ...
        void setDimensino(int dimension){
            this->dimension = dimension;
        }
        ...
        double calculateDistance(point b){
            ...
            for(int i = 0; i < this->dimension; i++){
                distance += (this->coordinates[i]-coordinatesB[i])*(this->coordinates[i]-coordinatesB[i]);;
            } 
...
void calculateClusters(vector<point>& points, const int npunti, const int nClusters, vector<point>& medoids, int* clusters, const int nthread) {
...
// - nthread: Number of threads for parallelization
void updateMedoids(vector<point>& points, const int npunti, vector<point>& medoids, int* clusters, const int nthread) {   
    int nClusters = medoids.size();
    double totalDistanceArray[nClusters];
    int medoidIndexArray[nClusters];

    // OpenMP parallelization for loop
    #pragma omp parallel default(shared) num_threads(nthread)
    {
...
long long kMedoids(vector<point>& points, const int npunti, const int nClusters, const int nthread) {
    ...
    for (iter = 0; iter < MAX_ITERATIONS; iter++) {
        calculateClusters(points, npunti, nClusters, medoids, clusters, nthread);
        updateMedoids(points, npunti, medoids, clusters, nthread);
    }
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0; i < NUM_EXECUTIONS; i++){
        ...
        long long execution_time = kMedoids(points, npunti, nClusters, nthread);
